...
virtual ~Scheduler();
//当前线程的调度器，同一个调度器下的所有线程指同同一个调度器实例
static Scheduler* Scheduler::GetThis(){return t_scheduler; }
// 当前线程的调度协程，每个线程都独有一份，包括caller线程
static Fiber* Scheduler::GetMainFiber(){ return t_scheduler_fiber; }
void start();//开始调度
void stop();//结束调度
...
- IO协程调度器继承直协程调度器，封装了`epoll`相关io事件，拥有协程调度器的所有功能，并具体实现了`tickle`方法以及`idle`方法，让`dile`协程阻塞在`eopll_wait`上,`tickle`可触发`epoll_wait`的返回，还对特定描述符进行事件注册，当描述符事件触发时，唤醒对应的协程。
